...
import "@stable-credit/contracts/interface/IStableCredit.sol";
...
contract LaunchPool is PausableUpgradeable {
    ...
    function launch() public onlyOperator notLaunched _canLaunch {
        ...
        stableCredit.assurancePool().reserveToken().approve(address(creditPool), totalDeposited);
    ...
    function depositTokens(uint256 amount) external virtual notLaunched whenNotPaused {
        ...
        stableCredit.assurancePool().reserveToken().transferFrom(
    ...
    function withdrawTokens(uint256 amount) external virtual whenNotPaused {
        ...
        stableCredit.assurancePool().reserveToken().transfer(_msgSender(), amount);
